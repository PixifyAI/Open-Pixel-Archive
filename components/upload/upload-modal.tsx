...
export function UploadModal({ onClose, onUploadComplete }: UploadModalProps) {
  ...
  const handleUpload = async () => {
    ...
    const uploadPromises = mainFilesToUpload.map(async ({ file, id }) => { // Destructure file and id
      const formData = new FormData();
      formData.append('file', file);

      // Append preview image if available for this file
      if (previewImages[id]) { // Use unique ID
        formData.append('previewImage', previewImages[id] as File); // Use unique ID
      }
      ...
      try {
        ...
        if (response.ok) {
          console.log(`File ${file.name} uploaded successfully. response.ok is: ${response.ok}`); // Log response.ok
          console.log(`Setting completed state for file ${file.name} with ID ${id} to true.`); // Log before setting completed with ID
          setCompleted(prev => ({
            ...prev,
            [id]: true // Use unique ID
          }));
          ...
          console.error(`Upload failed for file ${file.name}. Response status: ${response.status}, status text: ${response.statusText}`); // Detailed error log
          toast({
            title: "Upload failed",
    ...
    console.log("All upload promises settled."); // Log after Promise.all

    setUploading(false);

    // Close modal and trigger gallery refresh after all promises settle
    console.log("Upload process completed, closing modal and triggering gallery refresh.");
    onClose();
    onUploadComplete(); // Call onUploadComplete after all promises settle

    // Check if all files were successfully completed after all promises settle
    const allSuccessful = Object.values(completed).every(value => value);
     if (allSuccessful && files.length > 0) { // Ensure there were files to upload
       toast({
        ...
        >
          ...
          {files.length === 0 ? (
            ...
            <div className="space-y-3 max-h-64 overflow-auto py-2">
              {files.map(({ file, id }, index) => { // Destructure file and id
            ...
            {!uploading && (
              <div className="space-y-2">
                <p className="text-sm text-muted-foreground">
                  Would you like to add these files to your gallery?
                </p>
